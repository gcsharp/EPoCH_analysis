...
raw_dat <- zap_labels(raw_dat) #n is 19243 (same as n in family_file and listed on closer website)

raw_dat <- raw_dat[which(raw_dat$amdres00==1&is.na(raw_dat$amdres00)==FALSE),] # select only those where the natural mother is the main respondent (vast majority of cases, lose 55 non-mothers, 691 missing status) now 18497

raw_dat <- raw_dat[raw_dat$nocmhh==1,]#select singletons/first borns in cohort (lose 251 children of a twin pair + 10 children of a triplet trio) n now 18241
