...
dat$covs_edu_father_highestlowest_binary[dat$covs_edu_father%in%c(1,2,3)] <- 0
dat$covs_edu_father_highestlowest_binary[dat$covs_edu_father==0] <- 1
dat$covs_edu_mother_highestlowest_binary <- NA
dat$covs_edu_mother_highestlowest_binary[dat$covs_edu_mother%in%c(1,2,3)] <- 0
dat$covs_edu_mother_highestlowest_binary[dat$covs_edu_mother==0] <- 1
dat$covs_occup_father_highestlowest_binary <- NA
dat$covs_occup_father_highestlowest_binary[dat$covs_occup_father%in%c(1,2,3)] <- 0
dat$covs_occup_father_highestlowest_binary[dat$covs_occup_father==0] <- 1
...
dat$alcohol_mother_binge1_binary[raw_dat$alc05ufr3m%in%c(6)]<-0
dat$alcohol_mother_binge2_binary <-NA
dat$alcohol_mother_binge2_binary[raw_dat$alc05u4thm%in%c(1,2,3,4,5)]<-1
dat$alcohol_mother_binge2_binary[raw_dat$alc05u4thm%in%c(6)]<-0
...
# This is a duplicated variable of 'caffeine_mother_total_thirdtrim_continuous', as we only have data for the third trimester
...
child_genetic <- genetic_data[[1]][,grep("childpc|BiBPersonID|prs",colnames(genetic_data[[1]]))]
mum_genetic <- genetic_data[[2]][,grep("mumpc|BiBPersonID|prs",colnames(genetic_data[[2]]))]
mum_genetic <- mum_genetic[duplicated(mum_genetic$BiBPersonID)==F,]
dat <- left_join(dat,child_genetic,by=c("epoch_child_id"="BiBPersonID"))
dat <- left_join(dat,mum_genetic,by=c("BiBMotherID"="BiBPersonID"))
...
##save summary
tableone::CreateTableOne(names(dat)[grep("continuous|binary|ordinal|covs_|neuro_|immuno_|anthro_|negcon_|cardio_",names(dat))],factorVars = names(dat)[grep("binary|ordinal",names(dat))],data=dat[-which(dat$multiple_pregnancy==1),])->TABLE1
...
# save ethnicity stratified datasets (both parents = same ethnicity)

dat_sa <- dat[which(dat$covs_ethnicityBIB_mother=="south asian"&dat$covs_ethnicityBIB_father=="south asian"),]
dat_we <- dat[which(dat$covs_ethnicityBIB_mother=="white european"&dat$covs_ethnicityBIB_father=="white european"),]

dat_sa <- dat_sa[,-grep(colnames(dat_sa),pattern="ethnicity")]
dat_we <- dat_we[,-grep(colnames(dat_we),pattern="ethnicity")]

saveRDS(dat_sa,"/Volumes/MRC-IEU-research/projects/ieu2/p5/015/working/data/bibsa/bibsa_pheno.rds")
saveRDS(dat_we,"/Volumes/MRC-IEU-research/projects/ieu2/p5/015/working/data/bibwe/bibwe_pheno.rds")
